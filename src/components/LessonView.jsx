...
export default function LessonView({ week, domainColor, domainId = 1 }) {
  ...
  const hasContent = week.vocabulary?.length || week.concepts?.length || week.sessions?.length
    ...
    return (
      <div style={{ textAlign: 'center', padding: '3.5rem 1rem' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: C.muted, maxWidth: '340px', margin: '0 auto' }}>
          This week's content will unlock as you progress through the program.
        </p>
      </div>
    )
  }
    ...
    <div>
      ...
      {week.sessions?.length > 0 && (
        <section style={{ marginBottom: '1.75rem' }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.65rem' }}>Study Sessions — Week {week.week}</p>
      ...
      {week.vocabulary?.length > 0 && (
        <section style={{ marginBottom: '1.75rem' }}>
          <p style={{ fontSize: '0.72rem', fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.65rem' }}>{week.vocabulary.length} Key Terms — Click to Expand</p>
