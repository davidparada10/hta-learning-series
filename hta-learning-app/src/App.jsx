    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.25rem' }}>
        {Object.entries(CAP_DATA).map(([key, d]) => (
          <button key={key} onClick={() => setType(key)} style={pill(type === key, d.color)}>{d.label}</button>
        ))}
      </div>
...
function Quiz() {
  ...
  const reset = () => { setIdx(0); setSel(null); setRev(false); setAns([]); setDone(false) }
...
// ─── LESSON VIEW ──────────────────────────────────────────────────────────────
function LessonView({ week, domainColor }) {
...
// ─── COMING SOON (domain overview) ───────────────────────────────────────────
function DomainComingSoon({ domain }) {
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#F0F3F7' }}>
      ...
      <header style={{ background: C.navy, color: 'white', padding: '0.85rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
          ...
          <div>
            <p style={{ margin: 0, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', opacity: 0.65 }}>HTA Construction & Development · Infinity</p>
            <h1 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 900, letterSpacing: '-0.01em' }}>Learning Series</h1>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <p style={{ margin: 0, fontSize: '0.7rem', opacity: 0.65 }}>David Parada</p>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 1.5rem' }}>
          ...
          {/* Tab bar (only for content weeks) */}
          {(hasContent || domain.id !== 1) && domain.id === 1 && (
